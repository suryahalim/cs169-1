class Hour < ActiveRecord::Base
    ...
    def validate_open_close_hours
        if (open == nil && close != nil) or (close == nil && open != nil)
            errors.add(:open, "hour nil, but close hour not nil or vice versa")
        end
        if open != nil && close != nil
            ...
            errors.add(:open, "hour not before close hour") if open > close
        end
    end
